    ...
    if network.is_transmission:
        for year in network.years:
            for day in network.days:
                ...
                for node_id in network.active_distribution_network_nodes:
                    ...
                    fix_or_set(model[year][day].shared_es_s_rated_fixed[shared_ess_idx], abs(candidate_solution[node_id][year]['s']) / s_base)
                    fix_or_set(model[year][day].shared_es_e_rated_fixed[shared_ess_idx], abs(candidate_solution[node_id][year]['e']) / s_base)
    else:
        ...
        for year in network.years:
            for day in network.days:
                ...
                fix_or_set(model[year][day].shared_es_s_rated_fixed[shared_ess_idx], abs(candidate_solution[tn_node_id][year]['s']) / s_base)
                fix_or_set(model[year][day].shared_es_e_rated_fixed[shared_ess_idx], abs(candidate_solution[tn_node_id][year]['e']) / s_base)
