...
def _build_model(network, params):
    ...
    if params.rg_curt:
        ...
        model.sg_sqr_curt = pe.Var(model.generators, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0)
        for g in model.generators:
            generator = network.generators[g]
            for s_m in model.scenarios_market:
                for s_o in model.scenarios_operation:
            ...
            for g in model.generators:
                ...
                for p in model.periods:
                    if generator.is_curtaillable() and params.rg_curt:
                        ...
                        if params.rg_curt:
                            sg_curt = sqrt(pe.value(model.sg_sqr_curt[g, s_m, s_o, p])) * network.baseMVA
