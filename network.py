...
# ======================================================================================================================
def _build_model_new_version(network, params):
    ...
    model.vmag_sqr = pe.Var(model.nodes, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=1.0, bounds=partial(vmag_sqr_bounds, network=network, params=params))
...
def _build_model_original(network, params):
...
def _build_model(network, params):
    ...
    if params.shared_ess_model == ESS_MODEL_LP_SIMPLIFIED_EXTENDED:
        model.shared_energy_storage_relax_ch = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sess_simplified_model_ch_rule, network=network))
        model.shared_energy_storage_relax_dch = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sess_simplified_model_dch_rule, network=network))
        model.shared_energy_storage_relax_comp = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sess_simplified_model_comp_rule, network=network))
...
def _run_smopf(network, model, params, from_warm_start=False):
    ...
    if params.solver_params.solver == 'ipopt':
        ...
        solver.options['dual_inf_tol'] = 2
        solver.options['constr_viol_tol'] = params.solver_params.solver_tol * 1e2
        solver.options['compl_inf_tol'] = params.solver_params.solver_tol * 1e2
            ...
            for g in model.generators:
                if network.generators[g].is_renewable():
                    for p in model.periods:
                        ...
                        if params.rg_curt:
                            total_renewable_gen_scenario['s'] += network.baseMVA * sqrt(abs(pe.value(model.sg_sqr[g, s_m, s_o, p])))
    ...
    for g in model.generators:
        if network.generators[g].is_renewable():
            for p in model.periods:
                ...
                if params.rg_curt:
                    total_renewable_gen['s'] += network.baseMVA * sqrt(abs(pe.value(model.sg_sqr[g, s_m, s_o, p])))
    return total_renewable_gen
