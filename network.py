...
def _build_model(network, params):
    ...
    model.fi_fj = pe.Var(model.nodes, model.nodes, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=1.0)
                ...
                for p in model.periods:
                    if branch.is_transformer:
                        ...
                        if params.transf_reg and branch.vmag_reg:
                            ...
                            model.r_sqr[i, s_m, s_o, p].setub(branch.ratio ** 2 + EQUALITY_TOLERANCE)
                            model.r_sqr[i, s_m, s_o, p].setlb(branch.ratio ** 2 - EQUALITY_TOLERANCE)
                    else:
                        model.r[i, s_m, s_o, p].setub(1.00 + EQUALITY_TOLERANCE)
                        model.r[i, s_m, s_o, p].setlb(1.00 - EQUALITY_TOLERANCE)
                        model.r_sqr[i, s_m, s_o, p].setub(1.00 + EQUALITY_TOLERANCE)
                        model.r_sqr[i, s_m, s_o, p].setlb(1.00 - EQUALITY_TOLERANCE)
    ...
    if params.es_reg:
        ...
        if params.slacks.ess.charging:
            model.slack_es_ch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, domain=pe.Reals, initialize=0.0)
            model.slack_es_dch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, domain=pe.Reals, initialize=0.0)
        ...
        for s_m in model.scenarios_market:
            for s_o in model.scenarios_operation:
                for p in model.periods:
                    ...
                    # vmag_sqr definition
                    model.voltage_cons.add(model.vmag_sqr[i, s_m, s_o, p] <= e_actual ** 2 + f_actual ** 2 + EQUALITY_TOLERANCE)
                    model.voltage_cons.add(model.vmag_sqr[i, s_m, s_o, p] >= e_actual ** 2 + f_actual ** 2 - EQUALITY_TOLERANCE)
            ...
            for s_m in model.scenarios_market:
                for s_o in model.scenarios_operation:
                    for p in model.periods:
                        ...
                        model.ei_fj[tnode_idx, fnode_idx, s_m, s_o, p].fixed = False
                        ...
                        model.voltage_bilinear_terms.add(model.ei_fj[fnode_idx, tnode_idx, s_m, s_o, p] >= model.e_actual[fnode_idx, s_m, s_o, p] * model.f_actual[tnode_idx, s_m, s_o, p] - EQUALITY_TOLERANCE)
                        model.voltage_bilinear_terms.add(model.ei_fj[tnode_idx, fnode_idx, s_m, s_o, p] <= model.e_actual[tnode_idx, s_m, s_o, p] * model.f_actual[fnode_idx, s_m, s_o, p] + EQUALITY_TOLERANCE)
                        model.voltage_bilinear_terms.add(model.ei_fj[tnode_idx, fnode_idx, s_m, s_o, p] >= model.e_actual[tnode_idx, s_m, s_o, p] * model.f_actual[fnode_idx, s_m, s_o, p] - EQUALITY_TOLERANCE)
                        ...
                        if branch.status:
                            if branch.fbus == node.bus_i or branch.tbus == node.bus_i:
                                ...
                                if branch.fbus == node.bus_i:
                                    ...
                                    ei_fj = model.ei_fj[fnode_idx, tnode_idx, s_m, s_o, p]
                                    ej_fi = model.ei_fj[tnode_idx, fnode_idx, s_m, s_o, p]

                                    Pi += branch.g * vmag_sqr * rij_sqr
                                    Pi -= rij * branch.g * (ei_ej + fi_fj)
                                    Pi -= rij * branch.b * (ej_fi - ei_fj)
                                    Qi -= (branch.b + branch.b_sh * 0.5) * vmag_sqr * rij_sqr
                                    Qi += rij * branch.b * (ei_ej + fi_fj)
                                    Qi -= rij * branch.g * (ej_fi - ei_fj)
                                else:
                                    ...
                                    fi_fj = model.fi_fj[tnode_idx, fnode_idx, s_m, s_o, p]
                                    ei_fj = model.ei_fj[tnode_idx, fnode_idx, s_m, s_o, p]
                                    ej_fi = model.ei_fj[fnode_idx, tnode_idx, s_m, s_o, p]
