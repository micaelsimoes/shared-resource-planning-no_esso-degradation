...
def _process_results(network, model, params, results=dict()):
    ...
    for s_m in model.scenarios_market:
        processed_results['relaxation_slacks']['interface'][s_m] = dict()
        for s_o in model.scenarios_operation:
            processed_results['relaxation_slacks']['interface'][s_m][s_o] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_up'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_down'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_up'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_down'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_up'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_down'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_up'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_down'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_up'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_down'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_s_up'] = dict()
            processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_s_down'] = dict()
            if network.is_transmission:
                for dn in model.active_distribution_networks:
                    node_id = network.active_distribution_network_nodes[dn]
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_up'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_down'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_up'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_down'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_up'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_down'][node_id] = []
                    for p in model.periods:
                        vmag_up = pe.value(model.slack_expected_interface_vmag_sqr_up[dn, s_m, s_o, p])
                        vmag_down = pe.value(model.slack_expected_interface_vmag_sqr_down[dn, s_m, s_o, p])
                        pf_p_up = pe.value(model.slack_expected_interface_pf_p_up[dn, s_m, s_o, p])
                        pf_p_down = pe.value(model.slack_expected_interface_pf_p_down[dn, s_m, s_o, p])
                        pf_q_up = pe.value(model.slack_expected_interface_pf_q_up[dn, s_m, s_o, p])
                        pf_q_down = pe.value(model.slack_expected_interface_pf_q_down[dn, s_m, s_o, p])
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_up'][node_id].append(vmag_up)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_down'][node_id].append(vmag_down)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_up'][node_id].append(pf_p_up)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_down'][node_id].append(pf_p_down)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_up'][node_id].append(pf_q_up)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_down'][node_id].append(pf_q_down)
                for e in model.shared_energy_storages:
                    node_id = network.shared_energy_storages[e].bus
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_up'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_down'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_up'][node_id] = []
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_down'][node_id] = []
                    for p in model.periods:
                        p_up = pe.value(model.slack_expected_shared_ess_p_up[e, s_m, s_o, p])
                        p_down = pe.value(model.slack_expected_shared_ess_p_down[e, s_m, s_o, p])
                        q_up = pe.value(model.slack_expected_shared_ess_q_up[e, s_m, s_o, p])
                        q_down = pe.value(model.slack_expected_shared_ess_q_down[e, s_m, s_o, p])
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_up'][node_id].append(p_up)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_down'][node_id].append(p_down)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_up'][node_id].append(q_up)
                        processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_down'][node_id].append(q_down)
            else:
                ...
                processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_down'][node_id] = []
                for p in model.periods:
                    vmag_up = pe.value(model.slack_expected_interface_vmag_sqr_up[s_m, s_o, p])
                    vmag_down = pe.value(model.slack_expected_interface_vmag_sqr_down[s_m, s_o, p])
                    pf_p_up = pe.value(model.slack_expected_interface_pf_p_up[s_m, s_o, p])
                    pf_p_down = pe.value(model.slack_expected_interface_pf_p_down[s_m, s_o, p])
                    pf_q_up = pe.value(model.slack_expected_interface_pf_q_up[s_m, s_o, p])
                    pf_q_down = pe.value(model.slack_expected_interface_pf_q_down[s_m, s_o, p])
                    ess_p_up = pe.value(model.slack_expected_shared_ess_p_up[s_m, s_o, p])
                    ess_p_down = pe.value(model.slack_expected_shared_ess_p_down[s_m, s_o, p])
                    ess_q_up = pe.value(model.slack_expected_shared_ess_q_up[s_m, s_o, p])
                    ess_q_down = pe.value(model.slack_expected_shared_ess_q_down[s_m, s_o, p])
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_up'][node_id].append(vmag_up)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['vmag_down'][node_id].append(vmag_down)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_up'][node_id].append(pf_p_up)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_p_down'][node_id].append(pf_p_down)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_up'][node_id].append(pf_q_up)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['pf_q_down'][node_id].append(pf_q_down)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_up'][node_id].append(ess_p_up)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_p_down'][node_id].append(ess_p_down)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_up'][node_id].append(ess_q_up)
                    processed_results['relaxation_slacks']['interface'][s_m][s_o]['ess_q_down'][node_id].append(ess_q_down)
