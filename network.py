...
def _build_model(network, params):
    ...
    model.shared_es_soc = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sess_balance_rule, network=network, params=params))
    ...
    if params.es_reg:
        ...
        model.energy_storage_soc_limits = pe.Constraint(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(ess_soc_limits_rule, network=network))
        if params.ess_model == ESS_MODEL_LP_RELAXED:
