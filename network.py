...
def _build_model_v2(network, params):
    ...
    # - Energy Storage devices
    if params.es_reg:
        model.es_soc = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=partial(soc_initialize, network=network), bounds=partial(soc_bounds, network=network))
    ...
    model.shared_es_s_rated_fixed = pe.Var(model.shared_energy_storages, domain=pe.NonNegativeReals, initialize=0.0)
    model.shared_es_e_rated_fixed = pe.Var(model.shared_energy_storages, domain=pe.NonNegativeReals, initialize=0.0)
    model.shared_es_soc = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=partial(shared_soc_init, network=network), bounds=partial(shared_soc_bounds, network=network))
    model.shared_es_sch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation,model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(shared_s_bounds, network=network))
    model.shared_es_pch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(shared_s_bounds, network=network))
    model.shared_es_qch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=0.0, bounds=partial(shared_q_bounds, network=network))
    model.shared_es_sdch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(shared_s_bounds, network=network))
    model.shared_es_pdch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(shared_s_bounds, network=network))
    model.shared_es_qdch = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=0.0, bounds=partial(shared_q_bounds, network=network))
    model.shared_es_pnet = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=0.0)
    model.shared_es_qnet = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=0.0)
    if params.slacks.shared_ess.complementarity:
        model.slack_shared_es_comp = pe.Var(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0)
    ...
    # - Generation
    if params.rg_curt:
        model.sg_sqr_upper = pe.Constraint(model.generators, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sg_sqr_upper_bound_rule, network=network))
        model.sg_sqr_lower = pe.Constraint(model.generators, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sg_sqr_lower_bound_rule, network=network))
        model.sg_abs_upper = pe.Constraint(model.generators, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sg_abs_upper_bound_rule, network=network))
        model.sg_abs_lower = pe.Constraint(model.generators, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sg_abs_lower_bound_rule, network=network))
    ...
    model.shared_energy_storage_sch_def = pe.Constraint(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_sch_def)
    model.shared_energy_storage_sdch_def = pe.Constraint(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_sdch_def)
    model.shared_energy_storage_ch_dch_exclusion = pe.Constraint(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sess_comp_rule, params=params))
    model.shared_energy_storage_balance = pe.Constraint(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=partial(sess_balance_rule, network=network))
    ...
    # Objective Function
    build_objective(model, network, params)
...
def _build_model_bck(network, params):
    ...
    if params.es_reg:
        model.es_soc = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=partial(soc_initialize, network=network), bounds=partial(soc_bounds, network=network))
        model.es_sch = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(s_bounds, network=network))
        model.es_pch = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(s_bounds, network=network))
        model.es_qch = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=0.0, bounds=partial(q_bounds, network=network))
        model.es_sdch = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(s_bounds, network=network))
        model.es_pdch = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.NonNegativeReals, initialize=0.0, bounds=partial(s_bounds, network=network))
        model.es_qdch = pe.Var(model.energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, domain=pe.Reals, initialize=0.0, bounds=partial(q_bounds, network=network))
    ...
    # - Flexible Loads -- Daily energy balance
    model.flex_energy_balance_lower = pe.Constraint(model.loads, model.scenarios_market, model.scenarios_operation, rule=flex_energy_balance_rule_lower)
    model.flex_energy_balance_upper = pe.Constraint(model.loads, model.scenarios_market, model.scenarios_operation, rule=flex_energy_balance_rule_upper)
    ...
    model.shared_es_soc_limit_lower = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_soc_lower_limit)
    model.shared_es_soc_limit_upper = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_soc_upper_limit)
    model.shared_es_pnet_lower = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_pnet_lower)
    model.shared_es_pnet_upper = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_pnet_upper)
    model.shared_es_qnet_lower = pe.Constraint(model.shared_energy_storages, model.scenarios_market, model.scenarios_operation, model.periods, rule=sess_qnet_lower)
    ...
    for s_m in model.scenarios_market:
        ...
        for s_o in model.scenarios_operation:
            omega_oper = network.prob_operation_scenarios[s_o]
            weight = compute_weight(network, omega_market, omega_oper)
            obj_scenario = 0.0
            ...
            if params.obj_type == OBJ_MIN_COST:
                for g in model.generators:
                    ...
                    if gen.is_controllable() and (network.is_transmission or gen.gen_type != GEN_REFERENCE):
                        for p in model.periods:
                            obj_scenario += network.cost_energy_p[s_m][p] * network.baseMVA * model.pg[g, s_m, s_o, p]

                if params.fl_reg:
                    for c in model.loads:
                        for p in model.periods:
                            obj_scenario += add_flex_cost(model, s_m, s_o, p, c, network.cost_flex[s_m][p], network.baseMVA)

                if params.l_curt:
                    for c in model.loads:
                        for p in model.periods:
                            obj_scenario += add_load_curtailment(model, s_m, s_o, p, c, model.cost_load_curtailment, network.baseMVA)

                if params.rg_curt:
                    for g in model.generators:
                        if network.generators[g].is_curtaillable():
                            for p in model.periods:
                                obj_scenario += add_gen_curtailment(model, s_m, s_o, p, g, model.cost_res_curtailment, network.baseMVA)

                if params.es_reg:
                    for e in model.energy_storages:
                        for p in model.periods:
                            obj_scenario += add_ess_usage(model, s_m, s_o, p, e, model.es_sch, model.es_sdch, model.penalty_ess_usage, network.baseMVA)

                for e in model.shared_energy_storages:
                    for p in model.periods:
                        obj_scenario += add_ess_usage(model, s_m, s_o, p, e, model.shared_es_sch, model.shared_es_sdch, model.penalty_ess_usage, network.baseMVA)
            ...
            # --- Congestion Management Objective ---
                ...
                if params.rg_curt:
                    for g in model.generators:
                        for p in model.periods:
                            obj_scenario += add_gen_curtailment(model, s_m, s_o, p, g, model.penalty_gen_curtailment, network.baseMVA)

                if params.l_curt:
                    for c in model.loads:
                        for p in model.periods:
                            obj_scenario += add_load_curtailment(model, s_m, s_o, p, c, model.penalty_load_curtailment, network.baseMVA)

                if params.fl_reg:
                    for c in model.loads:
                        for p in model.periods:
                            obj_scenario += add_flex_cost(model, s_m, s_o, p, c, model.penalty_flex_usage, network.baseMVA)
            ...
            # Grid Operation Slacks
            if params.slacks.grid_operation.voltage:
                for i in model.nodes:
                    for p in model.periods:
                        obj_scenario += PENALTY_VOLTAGE * weight * (
                                add_slack_squared(model.slack_e[i, s_m, s_o, p]) +
                                add_slack_squared(model.slack_f[i, s_m, s_o, p])
                        )
            if params.slacks.grid_operation.branch_flow:
                for b in model.branches:
                    for p in model.periods:
                        obj_scenario += PENALTY_CURRENT * weight * model.slack_flow_ij_sqr[b, s_m, s_o, p]

            # Node Balance Slacks
            if params.slacks.node_balance:
                for i in model.nodes:
                    for p in model.periods:
                        obj_scenario += PENALTY_NODE_BALANCE * weight * (add_slack_squared(model.slack_node_balance_p[i, s_m, s_o, p]) + add_slack_squared(model.slack_node_balance_q[i, s_m, s_o, p]))

            # Flexibility Slacks
            if params.fl_reg and params.slacks.flexibility.day_balance:
                for c in model.loads:
                    obj_scenario += PENALTY_FLEXIBILITY * weight * add_slack_squared(model.slack_flex_p_balance[c, s_m, s_o])
            ...
            if params.es_reg:
                for e in model.energy_storages:
                    for p in model.periods:
                        if params.slacks.ess.complementarity:
                            obj_scenario += PENALTY_ESS * weight * model.slack_es_comp[e, s_m, s_o, p]
                    if params.slacks.ess.day_balance:
                        obj_scenario += PENALTY_ESS * weight * add_slack_squared(model.slack_es_soc_final[e, s_m, s_o])

            for e in model.shared_energy_storages:
                for p in model.periods:
                    if params.slacks.shared_ess.complementarity:
                        obj_scenario += PENALTY_SHARED_ESS * weight * model.slack_shared_es_comp[e, s_m, s_o, p]
                if params.slacks.shared_ess.day_balance:
                    obj_scenario += PENALTY_SHARED_ESS * weight * add_slack_squared(model.slack_shared_es_soc_final[e, s_m, s_o])

            # Accumulate scenario contribution
            obj += obj_scenario * omega_market * omega_oper
...
def _process_results(network, model, params, results=dict()):
    ...
    for s_m in model.scenarios_market:
        ...
        for s_o in model.scenarios_operation:
            ...
            if params.rg_curt:
                processed_results['scenarios'][s_m][s_o]['generation']['pg_net'] = dict()
                processed_results['scenarios'][s_m][s_o]['generation']['qg_net'] = dict()
                processed_results['scenarios'][s_m][s_o]['generation']['sg_net'] = dict()
            ...
            for g in model.generators:
                ...
                processed_results['scenarios'][s_m][s_o]['generation']['sg'][gen_id] = []
                if params.rg_curt:
                    processed_results['scenarios'][s_m][s_o]['generation']['pg_net'][gen_id] = []
                    processed_results['scenarios'][s_m][s_o]['generation']['qg_net'][gen_id] = []
                ...
                for p in model.periods:
                    ...
                    processed_results['scenarios'][s_m][s_o]['generation']['sg'][gen_id].append(sg)
...
def _process_results_summary_detail(network, model, params):
    ...
    for s_m in model.scenarios_market:
        ...
        results['scenarios'][s_m] = dict()

        for s_o in model.scenarios_operation:

            omega_o = network.prob_operation_scenarios[s_o]
            results['scenarios'][s_m][s_o] = dict()

            results['scenarios'][s_m][s_o]['probability'] = omega_m * omega_o

            results['scenarios'][s_m][s_o]['obj'] = _compute_objective_function_value_per_scenario(network, model, params, s_m, s_o)
            results['scenarios'][s_m][s_o]['load'] = _compute_load_per_scenario(network, model, params, s_m, s_o)
            results['scenarios'][s_m][s_o]['load_curtailed'] = _compute_load_curtailment_per_scenario(network, model, params, s_m, s_o)
            results['scenarios'][s_m][s_o]['flexibility'] = _compute_flexibility_per_scenario(network, model, params, s_m, s_o)
    ...
    if params.obj_type == OBJ_MIN_COST:
        ...
        for g in model.generators:
            if network.generators[g].is_controllable():
                ...
                for p in model.periods:
                    pg = pe.value(model.pg[g, s_m, s_o, p])
                    obj += c_p[s_m][p] * network.baseMVA * pg
...
def _compute_load_per_scenario(network, model, params, s_m, s_o):
    total_load = {'p': 0.00, 'q': 0.00}
    for c in model.loads:
        for p in model.periods:
...
def _compute_generation_per_scenario(network, model, s_m, s_o):
    total_gen = {'p': 0.00, 'q': 0.00}
    for g in model.generators:
        for p in model.periods:
...
def _compute_renewable_generation_per_scenario(network, model, params, s_m, s_o):
    total_renewable_gen = {'p': 0.00, 'q': 0.00, 's': 0.00}
    for g in model.generators:
        if network.generators[g].is_renewable():
            for p in model.periods:
                ...
                if params.rg_curt:
                    total_renewable_gen['s'] += network.baseMVA * sqrt(pe.value(model.sg_sqr[g, s_m, s_o, p]))
    return total_renewable_gen
...
def _compute_losses_per_scenario(network, model, params, s_m, s_o):
    power_losses = 0.0
    for k in model.branches:
        for p in model.periods:
            power_losses += _get_branch_power_losses(network, params, model, k, s_m, s_o, p)
    return power_losses
