...
def _run_operational_planning(planning_problem, candidate_solution, debug_flag=False):
    ...
    dso_models, initial_pf = create_distribution_networks_models(distribution_networks, candidate_solution['total_capacity'])
    tso_model = create_transmission_network_model(transmission_network, initial_pf, candidate_solution['total_capacity'])
...
def create_transmission_network_model(transmission_network, initial_pf, candidate_solution):
        ...
        for day in transmission_network.days:
            ...
            for dn in tso_model[year][day].active_distribution_networks:
                ...
                for p in tso_model[year][day].periods:
                    ...
                    if transmission_network.params.relax_equalities:
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_interface_vmag_sqr[dn, p] <= expected_vmag_sqr + EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_interface_vmag_sqr[dn, p] >= expected_vmag_sqr - EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_interface_pf_p[dn, p] <= expected_pf_p + EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_interface_pf_p[dn, p] >= expected_pf_p - EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_interface_pf_q[dn, p] <= expected_pf_q + EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_interface_pf_q[dn, p] >= expected_pf_q - EQUALITY_TOLERANCE)
                    else:
            ...
            for e in tso_model[year][day].shared_energy_storages:
                for p in tso_model[year][day].periods:
                    ...
                    if transmission_network.params.relax_equalities:
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_shared_ess_p[e, p] <= expected_sess_p + EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_shared_ess_p[e, p] >= expected_sess_p - EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_shared_ess_q[e, p] <= expected_sess_q + EQUALITY_TOLERANCE)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_shared_ess_q[e, p] >= expected_sess_q - EQUALITY_TOLERANCE)
                    else:
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_shared_ess_p[e, p] == expected_sess_p)
                        tso_model[year][day].expected_interface_values.add(tso_model[year][day].expected_shared_ess_q[e, p] == expected_sess_q)
...
def create_distribution_networks_models(distribution_networks, candidate_solution):
    ...
    initial_pf = get_normalization_values(distribution_networks, dso_models)

    return dso_models, initial_pf
    ...
    for node_id in distribution_networks:
        ...
        for year in distribution_network.years:
            ...
            for day in distribution_network.days:
                ...
                initial_values[node_id][year][day]['v'] = [1.00 for _ in range(distribution_network.num_instants)]
                ...
                for p in dso_models[node_id][year][day].periods:
                    interface_vmag = sqrt(pe.value(dso_models[node_id][year][day].expected_interface_vmag_sqr[p]))
...
def update_transmission_model_to_admm(transmission_network, model, initial_pf, params):
        ...
        for year in distribution_network.years:
            for day in distribution_network.days:
                ...
                for p in dso_model[year][day].periods:
                    ...
                    init_p = abs(initial_pf[node_id][year][day]['p'][p]) / s_base
                    if isclose(init_p, 0.00, abs_tol=SMALL_TOLERANCE):
                        init_p = 1.00

                    init_q = abs(initial_pf[node_id][year][day]['q'][p]) / s_base
                    if isclose(init_q, 0.00, abs_tol=SMALL_TOLERANCE):
                        init_q = 1.00
        ...
        for y in model.years:
            for d in model.days:
                for p in model.periods:
                    constraint_p_req = (model.es_pnet[e, y, d, p] - model.p_req[e, y, d, p])
                    constraint_q_req = (model.es_qnet[e, y, d, p] - model.q_req[e, y, d, p])
