    ...
    for node_id in distribution_networks:
        ...
        for year in distribution_network.years:
            for day in distribution_network.days:
                ...
                # Update costs (penalties) for the coordination procedure
                dso_model[year][day].penalty_ess_usage.fix(SMALL_TOLERANCE)
                if distribution_network.params.obj_type == OBJ_MIN_COST:
                    dso_model[year][day].cost_res_curtailment.fix(SMALL_TOLERANCE)
                    ...
                    dso_model[year][day].penalty_gen_curtailment.fix(PENALTY_GENERATION_CURTAILMENT)
                    dso_model[year][day].penalty_load_curtailment.fix(PENALTY_LOAD_CURTAILMENT)
                    dso_model[year][day].penalty_flex_usage.fix(SMALL_TOLERANCE)
