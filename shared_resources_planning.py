...
def update_shared_energy_storage_model_to_admm(shared_ess_data, model, params):
    ...
    model.p_req_tso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)            # Active power
    model.q_req_tso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)            # Reactive power
    model.p_req_dso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)            # Active power
    model.q_req_dso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)            # Reactive power
    model.dual_p_req_tso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)       # Dual variables
    model.dual_q_req_dso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)       # Dual variables
    model.dual_p_req_dso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)  # Dual variables
    model.dual_q_req_dso = pe.Var(model.energy_storages, model.years, model.days, model.periods, domain=pe.Reals)  # Dual variables
        ...
        for y in model.years:
            ...
            for d in model.days:
                for p in model.periods:
                    constraint_p_req_tso = (model.es_pnet[e, y, d, p] - model.p_req_tso[e, y, d, p]) / (2 * rating_s)
                    constraint_q_req_tso = (model.es_qnet[e, y, d, p] - model.q_req_tso[e, y, d, p]) / (2 * rating_s)
                    constraint_p_req_dso = (model.es_pnet[e, y, d, p] - model.p_req_dso[e, y, d, p]) / (2 * rating_s)
                    constraint_q_req_dso = (model.es_qnet[e, y, d, p] - model.q_req_dso[e, y, d, p]) / (2 * rating_s)
        ...
        for y in model.years:
            ...
            for d in model.days:
                ...
                for p in model.periods:
                    ...
                    dual_p_req_tso = dual_ess['current']['tso'][node_id][year][day]['p'][p]
                    dual_q_req_tso = dual_ess['current']['tso'][node_id][year][day]['q'][p]
                    p_req_dso = ess_req['dso']['current'][node_id][year][day]['p'][p]
                    q_req_dso = ess_req['dso']['current'][node_id][year][day]['q'][p]
                    dual_p_req_dso = dual_ess['current']['dso'][node_id][year][day]['p'][p]
                    dual_q_req_dso = dual_ess['current']['dso'][node_id][year][day]['q'][p]
