        ...
        for y in esso_model.years:
            ...
            for d in esso_model.days:
                ...
                for p in esso_model.periods:
                    sreq = (sess_vars['tso']['current'][node_id][year][day]['s'][p] + sess_vars['dso']['current'][node_id][year][day]['s'][p]) * 0.50
                    esso_model.es_snet[e, y, d, p].fix(sreq)
        ...
        for y in esso_model.years:
            ...
            for d in esso_model.days:
                ...
                for p in esso_model.periods:
                    ...
                    sess_vars['esso']['current'][node_id][year][day]['s'][p] = shared_ess_s
                    sess_vars['esso']['prev'][node_id][year][day]['s'][p] = shared_ess_s
    ...
    for dn in range(len(planning_problem.active_distribution_network_nodes)):
        ...
        for year in planning_problem.years:
            ...
            for day in planning_problem.days:
                ...
                consensus_variables['ess']['esso']['current'][node_id][year][day] = {'s': [0.0] * num_instants}
                ...
                consensus_variables['ess']['esso']['prev'][node_id][year][day] = {'s': [0.0] * num_instants}
                ...
                dual_variables['ess']['esso']['current']['tso'][node_id][year][day] = {'s': [0.0] * num_instants}
                dual_variables['ess']['esso']['current']['dso'][node_id][year][day] = {'s': [0.0] * num_instants}
                ...
                dual_variables['ess']['esso']['prev'][node_id][year][day] = {'s': [0.0] * num_instants}
        ...
        for y in sess_model.years:
            ...
            for d in sess_model.days:
                ...
                for p in sess_model.periods:
                    s_req = pe.value(sess_model.es_snet[shared_ess_idx, y, d, p])
                    shared_ess_vars['esso']['current'][node_id][year][day]['s'][p] = min(max(s_req, -s_max), s_max)
        ...
        for y in range(len(repr_years)):
            ...
            for d in range(len(repr_days)):
                ...
                for p in tso_model[year][day].periods:
                    ...
                    s_req = pe.value(tso_model[year][day].expected_shared_ess_s[shared_ess_idx, p]) * s_base
                    ...
                    shared_ess_vars['tso']['current'][node_id][year][day]['s'][p] = min(max(s_req, -s_max), s_max)
        ...
        for y in range(len(repr_years)):
            ...
            for d in range(len(repr_days)):
                ...
                for p in dso_model[year][day].periods:
                    ...
                    s_req = pe.value(dso_model[year][day].expected_shared_ess_s[p]) * s_base
                    ...
                    shared_ess_vars['dso']['current'][node_id][year][day]['s'][p] = min(max(s_req, -s_max), s_max)
        ...
        for year in planning_problem.years:
            for day in planning_problem.days:
                ...
                for p in range(planning_problem.num_instants):
                    ...
                    sum_abs += rho_tso_ess * abs(round(consensus_vars['ess']['tso']['current'][node_id][year][day]['s'][p], ERROR_PRECISION) - round(consensus_vars['ess']['tso']['prev'][node_id][year][day]['s'][p], ERROR_PRECISION))
                    ...
                    sum_abs += rho_dso_ess * abs(round(consensus_vars['ess']['dso']['current'][node_id][year][day]['s'][p], ERROR_PRECISION) - round(consensus_vars['ess']['dso']['prev'][node_id][year][day]['s'][p], ERROR_PRECISION))
                    sum_abs += rho_esso * abs(round(consensus_vars['ess']['esso']['current'][node_id][year][day]['s'][p], ERROR_PRECISION) - round(consensus_vars['ess']['esso']['current'][node_id][year][day]['s'][p], ERROR_PRECISION))
                    num_elems += 7
    ...
    for node_id in distribution_networks:
        ...
        for year in distribution_network.years:
            for day in distribution_network.days:
                ...
                dso_model[year][day].v_sqr_req = pe.Var(dso_model[year][day].periods, domain=pe.NonNegativeReals)
                dso_model[year][day].dual_v_sqr_req = pe.Var(dso_model[year][day].periods, domain=pe.Reals)   # Dual variable - active power
                dso_model[year][day].v_sqr_prev = pe.Var(dso_model[year][day].periods, domain=pe.Reals)    # Active power - previous iteration
                dso_model[year][day].dual_v_sqr_prev = pe.Var(dso_model[year][day].periods, domain=pe.Reals)  # Dual variable - previous iteration
        ...
        for y in model.years:
            for d in model.days:
                for p in model.periods:
                    model.es_snet[e, y, d, p].fixed = False
    ...
    for e in model.energy_storages:
        for y in model.years:
            ...
            for d in model.days:
                day = days[d]
                for p in model.periods:
                    s_req = (ess_req['tso']['current'][node_id][year][day]['s'][p] + ess_req['dso']['current'][node_id][year][day]['s'][p]) * 0.50
                    dual_s_req = (dual_ess['esso']['current']['tso'][node_id][year][day]['s'][p] + dual_ess['esso']['current']['dso'][node_id][year][day]['s'][p]) * 0.50
                    model.s_req[e, y, d, p].fix(s_req)
                    model.dual_s_req[e, y, d, p].fix(dual_s_req)
